...
namespace MiniORM
{
    ...
    public abstract class DbContext
    {
        ...
        private bool IsObjectValid(object ent)
        {
            var validationContext = new ValidationContext(ent);
            var validationErrors = new List<ValidationResult>();
            var validationResult = Validator.TryValidateObject(ent, validationContext, validationErrors, validateAllProperties: true);
            ...
            where TDbSet : class, new() where TCollection : class, new()
        {
            ...
            foreach ( var entity in dbSet)
            {
                ...
                var navigationEntities = navigationDbSet
                    .Where(navEnt => primaryKey.GetValue(navEnt).Equals(primaryKeyValue))
                    .ToArray();
            ...
            foreach (var foreignKey in foreignKeys)
            {
                ...
                foreach (var entity in dbSet)
                {
                    ...
                    var navigationPropertyValue = ((IEnumerable<object>)navigationDbSet)
                        .First(curNavProp => navigationPrimaryKey.GetValue(curNavProp).Equals(foreignKeyValue));
        ...
        private void InitializeDbSets()
        {
            foreach (var dbSet in this.dbSetProperties)
            {
                ...
                var dbSetPropertyType = dbSet.Value;
        ...
        {
            var tableNames = this.GetTableName(table);

            var dbColumns = this.connection.FetchColumnNames(tableNames);
